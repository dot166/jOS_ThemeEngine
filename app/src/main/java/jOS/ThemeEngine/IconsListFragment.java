...
public class IconsListFragment extends Fragment implements IconSelectListener {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            icons = IconUtils.ParseIconPack(requireContext(), "jOS.ThemeEngine", res);
        } catch (Exception e) {
            ErrorUtils.handle(e, requireActivity());
            icons = new ArrayList<IconUtils.Icon>();
        }

        icons.add(new IconUtils.Icon(AppCompatResources.getDrawable(requireContext(), jOS.Core.R.drawable.ic_launcher_j), requireActivity().getComponentName()));
